...
static void espidf_delay_ms(uint32_t ms) {
    /* pdMS_TO_TICKS can round to 0 when ms < portTICK_PERIOD_MS (e.g. 1 ms at
       the default 100 Hz tick rate gives 0 ticks).  vTaskDelay(0) does NOT
       yield, so the IDLE task never runs and the task watchdog fires.
       Clamp to at least 1 tick so the scheduler always gets CPU time. */
